...
let handler = async (m, { conn, command, args }) => {
    ...
    if (command === 'fut') {
        ...
        for (let type of ['bronze', 'silver', 'gold']) {
            if (data.fifaInventory[type] > 0) {
                ...
                buttons.push({
                    buttonId: `.open ${type}`,
                    buttonText: { displayText: global.t(`futOpen${type.charAt(0).toUpperCase() + type.slice(1)}`, userId, groupId) },
                    type: 1
                });
            }
        }
        ...
        if (buttons.length === 0) {
            buttons.push({
                buttonId: '.futstore',
                buttonText: { displayText: global.t('futBuyStore', userId, groupId) },
                type: 1
            });
        }
    ...
    if (command === 'futstore') {
        const txt = global.t('futStorePrices', userId, groupId, {
        ...
        return conn.sendMessage(m.chat, {
            text: global.t('futStoreTitle', userId, groupId) + '\n' + txt,
    ...
    if (command === 'futbuy') {
        ...
        if (data.limit < prices[type]) {
            return m.reply(global.t('futBuyInsufficient', userId, groupId, { price: prices[type], type: type }));
        }

        data.limit -= prices[type];
        data.money = data.limit;
        data.fifaInventory[type]++;
        return m.reply(global.t('futBuySuccess', userId, groupId, { type: type, count: data.fifaInventory[type] }));
    }

    if (command === 'open') {
        const type = args[0]?.toLowerCase();
        if (!['bronze', 'silver', 'gold'].includes(type)) return m.reply('❌ Specifica il pacchetto da aprire.');
        if (data.fifaInventory[type] <= 0) return m.reply(global.t('futOpenNoPack', userId, groupId, { type: type }));

        data.fifaInventory[type]--;
        await conn.sendMessage(m.chat, { text: global.t('futOpenSuccess', userId, groupId, { type: type }) }, { quoted: m });
        ...
        for (let [i, p] of cards.entries()) {
            if (i === 0) {
                ...
                await conn.sendMessage(m.chat, {
                    text: global.t('futAdditionalPlayer', userId, groupId, { name: p.name, rating: p.rating })
                }, { quoted: m });
            }
        }
