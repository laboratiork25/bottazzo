...
export async function handler(m, { conn, text, command, isAdmin, isOwner }) {
    ...
    if (['setbenvenuto', 'setwelcome', 'benvenuto'].includes(cmd)) {
        if (!text) {
            const message = global.t('welcomeSetHelp', m.sender, m.isGroup ? m.chat : null, { command });
        ...
        if (text.toLowerCase() === 'reset') {
            ...
            const message = global.t('welcomeReset', m.sender, m.isGroup ? m.chat : null);
            return m.reply(message);
        }
        
        chat.customWelcome = text
        global.db.data.chats[m.chat] = chat
        const message = global.t('welcomeSet', m.sender, m.isGroup ? m.chat : null);
        return m.reply(message);
        
    } else if (['setaddio', 'setgoodbye', 'addio'].includes(cmd)) {
        if (!text) {
            const message = global.t('goodbyeSetHelp', m.sender, m.isGroup ? m.chat : null, { command });
            return m.reply(message);
        }
        ...
        if (text.toLowerCase() === 'reset') {
            ...
            global.db.data.chats[m.chat] = chat
            const message = global.t('goodbyeReset', m.sender, m.isGroup ? m.chat : null);
            return m.reply(message);
        }
        
        chat.customGoodbye = text
        global.db.data.chats[m.chat] = chat
        const message = global.t('goodbyeSet', m.sender, m.isGroup ? m.chat : null);
        return m.reply(message);
