...
let handler = async (m, { conn, args, participants, isAdmin, isBotAdmin }) => {
  if (m.text?.toLowerCase() === '.skipbandiera') {
    ...
    if (!m.isGroup) return m.reply(global.t('smsOnlyGroup', userId, groupId))
    if (!global.bandieraGame?.[m.chat]) return m.reply(global.t('flagNoActiveGame', userId, groupId))
    
    if (!isAdmin && !m.fromMe) {
      return m.reply(global.t('adminOnly', userId, groupId))
    }

    clearTimeout(global.bandieraGame[m.chat].timeout)
    await conn.reply(m.chat, global.t('flagGameInterrupted', userId, groupId, { answer: global.bandieraGame[m.chat].risposta }), m)
  ...
  if (now - lastGame < cooldownTime) {
    ...
    return m.reply(global.t('flagCooldown', m.sender, m.isGroup ? m.chat : null, { seconds: remainingTime }))
  }
  ...
  try {
    ...
    global.bandieraGame[m.chat] = {
      ...
      timeout: setTimeout(() => {
        if (global.bandieraGame?.[m.chat]) {
          conn.reply(m.chat, global.t('flagTimeExpired', m.sender, m.isGroup ? m.chat : null, { answer: scelta.nome }), msg)
          delete global.bandieraGame[m.chat]
        }
      }, 30000)
    }
...
> \`vare ✧ bot\``
    ...
    game.suggerito = true
    await conn.reply(chat, global.t('flagAlmostThere', m.sender, m.isGroup ? m.chat : null), m)
    
  } else if (game.tentativi[m.sender] >= 3) {
    await conn.reply(chat, global.t('flagAttemptsExhausted', m.sender, m.isGroup ? m.chat : null), m)
...
💡 *Suggerimento:*
   ...
   • È composta da *${numeroLettere} lettere*`, m)
      ...
      await conn.reply(chat, global.t('flagWrongAnswerTryAgain', m.sender, m.isGroup ? m.chat : null), m)
    } else {
      await conn.reply(chat, global.t('flagWrongAnswerLastTry', m.sender, m.isGroup ? m.chat : null), m)
    }
  }
}
