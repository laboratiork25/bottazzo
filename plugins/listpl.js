...
let handler = async (m, { __dirname, args }) => {
    try {
        ...
        if (jsFiles.length === 0) {
            return m.reply(global.t('noJsPluginsFound', m.sender, m.chat));
        }
        ...
        if (searchTerm) {
            ...
            if (pluginInfos.length === 0) {
                return m.reply(global.t('noPluginsFoundFor', m.sender, m.chat, { searchTerm: toBold(searchTerm) }));
            }
        }

        pluginInfos.sort((a, b) => a.name.localeCompare(b.name));
        
        let response = global.t('availablePlugins', m.sender, m.chat, { count: pluginInfos.length, bold: toBold });
        ...
        response += `\n${global.t('useListplDForDetails', m.sender, m.chat, { bold })}`;
        response += `\n${global.t('useListplSearch', m.sender, m.chat, { bold })}`;
