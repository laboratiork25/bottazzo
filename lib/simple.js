/**
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║                        CHATUNITY BOT - SIMPLE LIBRARY                       ║
 * ║                                                                              ║
 * ║  ► Versione: 7.0 Beta                                                       ║
 * ║  ► Sviluppatore: Army                                                       ║
 * ║  ► Update: Button Response Handler & Enhanced Message Parser                ║
 * ║  ► Data: 2025                                                               ║
 * ║                                                                              ║
 * ║  Libreria principale per la gestione dei messaggi WhatsApp con supporto    ║
 * ║  per bottoni interattivi e parsing avanzato dei messaggi.                  ║
 * ╚══════════════════════════════════════════════════════════════════════════════╝
 */
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
		...
		text: {
			get() {
                ...
                if (this.mtype === 'templateButtonReplyMessage' && this.msg?.selectedId) {
                    return this.msg.selectedId
                }
                if (this.mtype === 'buttonsResponseMessage' && this.msg?.selectedButtonId) {
                    return this.msg.selectedButtonId
                }
                if (this.mtype === 'interactiveResponseMessage' && this.msg?.nativeFlowResponseMessage?.paramsJson) {
                    try {
                        const params = JSON.parse(this.msg.nativeFlowResponseMessage.paramsJson)
                        return params.id
                    } catch (e) {
